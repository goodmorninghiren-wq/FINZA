...
const getClient = async (supabase: any, redirectUri?: string) => {
    const { data: { user } } = await supabase.auth.getUser();
    
    let clientId = process.env.NEXT_PUBLIC_QBO_CLIENT_ID || 'PLACEHOLDER';
    let clientSecret = process.env.QBO_CLIENT_SECRET || 'PLACEHOLDER';
    let environment = process.env.NEXT_PUBLIC_QBO_ENVIRONMENT || 'sandbox';
    ...
    // Use dynamic redirectUri if provided (preferred), fall back to env var
    ...
    console.log(`[QBO Client] Redirect URI: ${finalRedirectUri}`);
...
export const qboClient = {
    getAuthUri: async (supabase: any, redirectUri?: string) => {
        const client = await getClient(supabase, redirectUri);
